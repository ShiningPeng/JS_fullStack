...
export default {
  ...
  data () {
    return {
      ...
      speedNow:12,
      kcalNow:30,
      distance:62,  //当前移动的公里数
      markText:'',    //备注
      stratShow:true,//是否开始跑步
      mapData:{}, //定位后所在的地理信息
      startBtn:'开始',
  ...
  methods:{
    ...
    createTrip(){
      Toast.loading('加载中...');
      let that = this;
      let buttonDom = document.getElementById('buttomDom');
      this.map = new window.AMap.Map('map', {
        resizeEnable:true, //是否监控地图容器的变化，map自带的
        zoom:11,  //初始化地图层级
      ...
      this.map.plugin(['AMap.Geolocation', 'AMap.ControlBar'], function(){
        ...
        //返回定位信息
        window.AMap.event.addListener(geolocation, 'complete', that.localOnComplete)
  ...
  mounted() {
    this.tripType = this.$route.params.tripType;//取到当前路由的参数中的tripType
    if(!this.tripType){
